...
What is the numerical system for site order?
Are there more of some species than others
how many unique species are there?
How many counts of each observation by legal status? Focus on the highest trees?

```{r}
sfo_trees_top <- sfo_trees_df |> 
  group_by(legal_status) |>
  summarize(tree_count=n()) |> #focus on tree count data
  slice_max(tree_count, n=5) |> #take the top 5
  ggplot() +
...
Search through all legal_status exactly equal to Permitted site as well as within the caretaker column whichever is MTA and DPW 

What does the `%in%` do? Write an example to explain the `%in%` operator to your neighbor using the empty code chunk below

Picks whichever objects are within vector from former frame
...
select columns of legal status, location, and planting date

filter for oak and pine trees

summarize counts of oak and pines
...
oak_pine_df <- sfo_trees_df |>
  filter(str_detect(species, "Oak") | str_detect(species, "Pine")) |> #look within column, anytime you see for Oak or Pine, or is made by the |
  select(species, legal_status, plant_date, latitude, longitude) |>
  mutate(type=ifelse(str_detect(species, "Oak"), "Oak", "Pine"))  #create logical test, if I see an Oak -> return Oak in column, otherwise return Pine
```
...
oak_pine_count<-oak_pine_df |> 
  group_by(legal_status,type) |> #out of legal statuses
  summarize(count=n())
```
...
```{r}
sfo_scpl_map <- ggplot(oak_pine_df, aes(x=longitude, y=latitude, color=type)) +
  geom_point() +
...
```{r}
lubridate::year("2022-11-16") #takes in info, and returns #
```

```{r}
sfo_trees_native<-sfo_trees_df |>
  separate(species, into=c("spp_sci", "spp_common"), sep=" :: ") |> #separate out species into two new columns, need to include spaces next to columns
...
st_crs(oak_pine_sf)<-4326 #specifically California
